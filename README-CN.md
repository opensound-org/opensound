...
## Rust版本策略
作为官方，我们始终使用最新稳定版的Rust（当前是1.77.2）来构建和测试本项目。但是任何高于[Cargo.toml](Cargo.toml)中指定的[MSRV](https://doc.rust-lang.org/stable/cargo/reference/manifest.html#the-rust-version-field)（当前是1.75.0，因为依赖项中MSRV最高的“[poem](https://crates.io/crates/poem)”的MSRV是1.75.0）的Rust版本，均应正常构建，但是行为是否正确，官方不予保证。
